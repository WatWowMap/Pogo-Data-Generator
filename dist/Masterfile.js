...
const pogo_protos_1 = __importDefault(require("pogo-protos"));
...
class Masterfile {
    ...
    ensureFormName(form, id, formName) {
        if (!form.name) {
            form.name = this.capitalize(formName.substr(id === this.PokemonList.NIDORAN_FEMALE || id === this.PokemonList.NIDORAN_MALE
                ? 8
    ...
    compileEvolutions(target, object, pokemon = target) {
    ...
    pokemon(options, template, object) {
        ...
        if (this.finalData.pokemon[this.keyRef[id]]) {
            if (template.forms) {
                ...
                if (obj.formId && template.forms) {
                    ...
                    const statChecker = obj.attack === primaryForm.attack &&
                    ...
                    if (template.forms.quickMoves === 'unique' &&
                    ...
                    if (template.forms.chargedMoves === 'unique' &&
                    ...
                    if (template.forms.types === 'unique' &&
                    ...
                    if (primaryForm.tempEvolutions && (obj.name === 'Normal' || obj.name === 'Purified')) {
                        obj.tempEvolutions = {};
                        Object.keys(primaryForm.tempEvolutions).forEach((evo) => (obj.tempEvolutions[evo] = {}));
                    }
            ...
            obj.family = this.FamilyId[object.data.pokemonSettings.familyId];
            obj.genId = +Object.keys(generations_json_1.default).find(gen => {
    ...
    addMissingPokemon(options, template) {
        Object.values(this.PokemonList).forEach((id) => {
            if (id) {
                const obj = { pokedexId: id };
                this.ensurePokemon(id, obj);
                if (!this.keyRef[id]) {
                    obj.defaultFormId = 0;
                    this.loopFields('pokemon', obj, options, template, false);
                ...
                if (template.tempEvolutions) {
                    ...
                    if (guessedMega) {
                        ...
                        for (const { tempEvoId, attack, defense, stamina, type1, type2 } of guessedMega) {
                            ...
                            else if (obj.tempEvolutions[tempEvoId].attack !== attack ||
                        ...
                        if (this.finalData.pokemon[this.keyRef[obj.pokedexId]].tempEvolutions) {
                            this.finalData.pokemon[this.keyRef[obj.pokedexId]].tempEvolutions = Object.assign(Object.assign({}, this.finalData.pokemon[this.keyRef[obj.pokedexId]].tempEvolutions), obj.tempEvolutions);
                        }
                        else {
                            this.finalData.pokemon[this.keyRef[obj.pokedexId]].tempEvolutions = Object.assign({}, obj.tempEvolutions);
                        }
                    }
                }
            }
    ...
    questRewardTypes(options, template) {
        Object.entries(this.QuestRewardTypes).forEach((type) => {
            const [proto, id] = type;
            const obj = {
                id,
                proto,
    ...
    questConditions(options, template) {
        Object.entries(this.QuestConditions).forEach((type) => {
            const [proto, id] = type;
            const obj = {
                id,
                proto,
    ...
    loopFields(category, obj, options, template, sub) {
        const newObj = {};
        try {
            Object.entries(sub ? template[sub.name] : template).forEach((option) => {
    ...
    keyResolver(options, obj, sub) {
        try {
            const split = sub ? options[`${sub.name}Key`].split(' ') : options.key.split(' ');
            let newKey = '';
            if (split.length === 1) {
                newKey = obj[split[0]];
            }
    ...
    invasions(options, template, data) {
        this.finalData.invasions = {};
        Object.entries(data).forEach(gruntType => {
            const id = +gruntType[0];
            const info = gruntType[1];
            const newGrunt = Object.assign(Object.assign({}, this.formatGrunts(info.character)), { id });
            if (info.active) {
    ...
    compile(pokemon, moves, items, questConditions, questRewardTypes) {
        this.MovesList = pogo_protos_1.default.Rpc.HoloPokemonMove;
        this.FormsList = pogo_protos_1.default.Rpc.PokemonDisplayProto.Form;
        this.PokemonList = pogo_protos_1.default.Rpc.HoloPokemonId;
        this.ItemList = pogo_protos_1.default.Rpc.Item;
        this.GenderList = pogo_protos_1.default.Rpc.PokemonDisplayProto.Gender;
        this.TempEvolutions = pogo_protos_1.default.Rpc.HoloTemporaryEvolutionId;
        this.FamilyId = pogo_protos_1.default.Rpc.HoloPokemonFamilyId;
        this.QuestRewardTypes = pogo_protos_1.default.Rpc.QuestRewardProto.Type;
        this.QuestConditions = pogo_protos_1.default.Rpc.QuestConditionProto.ConditionType;
        if (pokemon && pokemon.enabled) {
            this.finalData.pokemon = {};
            this.formsToSkip = pokemon.options.skipForms.map((name) => name.toUpperCase());
        }
